...
function subscribeMessages() {
  ...
  _msgRef.on('child_added', snap => {
    ...
    // Solo incrementar "no leídos" para mensajes posteriores al inicio de sesión
    if (from !== driverUnit && (m.timestamp || 0) > _sessionStart) {
      unread++;
      updateUnreadBadge();
    }
  });
...
function addMsg(from, text, cls) {
  ...
  d.className = 'msg ' + (cls || '');
  d.innerHTML = `<div class="msg-from">${_escHtml(from)}</div><div class="msg-txt">${_escHtml(text)}</div>`;
  area.appendChild(d);
