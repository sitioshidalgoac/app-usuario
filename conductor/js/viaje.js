...
function mostrarViajeAsignado(viajeData, viajeId) {
  ...
  _countdownInt = setInterval(() => {
    ...
    if (segundos <= 0) { clearInterval(_countdownInt); rechazarViaje('expirada'); }
  }, 1000);
...
function aceptarViaje() {
  ...
  console.log('[ACEPTAR] start', {
    viajeId:   _viajeActualId,
    dbReady:   !!db,
    driverUnit: driverUnit,
    authEmail: _currentUser ? _currentUser.email : null,
    authUid:   _currentUser ? _currentUser.uid   : null,
  });

  // Watchdog: si la transacción no resuelve en 10 s, reportar en consola.
  let _txResolved = false;
  const _txWatchdog = setTimeout(() => {
    if (!_txResolved) {
      console.error('[TX TIMEOUT 10s] La transacción no resolvió. Estado Firebase pendiente.',
        { viajeId: _viajeActualId, driverUnit });
    }
  }, 10000);
  // ────────────────────────────────────────────────────────────────────────────
  ...
  solicitudRef.transaction(current => {
    console.log('[TX in]', JSON.stringify(current));

    if (!current) {
      console.log('[TX abort] nodo null — nodo eliminado o sin caché local');
      return; // undefined cancela la transacción sin borrar
    }
    if (current.estado !== 'enviado' || current.conductorId) {
      console.log('[TX abort] ya tomado o estado incorrecto', { estado: current.estado, conductorId: current.conductorId });
    ...
    console.log('[TX merge]', JSON.stringify(merged));
    return merged;

  }, (error, committed, snapshot) => {
    _txResolved = true;
    clearTimeout(_txWatchdog);

    console.log('[TX out]', {
      error:       error ? error.message : null,
      errorCode:   error ? error.code    : null,
      committed,
      finalEstado: snapshot && snapshot.val() ? snapshot.val().estado : null,
    });
    ...
    const solId = _viajeActualId; // capturar antes de que cerrarModalViaje lo nule

    try {
      setStatus('OCUPADO');
    } catch (e) {
      console.error('[ACEPTAR] setStatus OCUPADO falló', e);
    }

    try {
      db.ref('solicitudes_archivo/' + solId)
        .set({ ..._vData, estado: 'aceptada', cerradaEn: firebase.database.ServerValue.TIMESTAMP, motivo: 'aceptada' })
        .then(() => db.ref('solicitudes/' + solId).remove())
        .catch(e => console.warn('archivo solicitud falló', e));
    } catch (e) {
      console.error('[ACEPTAR] archive falló', e);
    }
...
function rechazarViaje(motivo) {
  if (_countdownInt) clearInterval(_countdownInt);
  if (db && _viajeActualId && _viajeActualData) {
    const solId   = _viajeActualId;
    const solData = _viajeActualData;
    db.ref('solicitudes_archivo/' + solId)
      .set({ ...solData, cerradaEn: firebase.database.ServerValue.TIMESTAMP, motivo: motivo || 'rechazada' })
      .then(() => db.ref('solicitudes/' + solId).remove())
      .catch(e => console.warn('archivo solicitud falló', e));
  }
  cerrarModalViaje();
...
function escucharSolicitudesAsignadas() {
  ...
  _solicitudesRef1.on('child_added', snap => {
    const v = snap.val();
    if (!v) return;
    if (v.estado !== 'enviado') return;
    if (v.conductorId && v.conductorId !== driverUnit) return;
    const edadMs = Date.now() - (v.timestamp || v.ts || 0);
    if (edadMs > 90000) return;
    if (myStatus === 'LIBRE') mostrarViajeAsignado(v, snap.key);
  });
  ...
  _solicitudesRef2.on('child_added', snap => {
    ...
    const edadMs = Date.now() - (v.timestamp || v.ts || 0);
    if (edadMs > 90000) return;
    if (!v.unidadId && myStatus === 'LIBRE' && !_viajeActualId) mostrarViajeAsignado(v, snap.key);
  });
}
...
function cargarCalificacionesConductor() {
  if (!db || !driverUnit) return;
  const email = 'unidad' + driverUnit.toLowerCase();
...
function btnCompletado() {
  if (!db || !viajeActivo || !viajeActivo.solicitudId) { toast('⚠️ Sin viaje activo', 'warn'); return; }
  const modal = document.getElementById('modal-confirmar-completado');
  if (modal) modal.style.display = 'flex';
}

function _ejecutarCompletado() {
  const modal = document.getElementById('modal-confirmar-completado');
  if (modal) modal.style.display = 'none';
  if (!db || !viajeActivo || !viajeActivo.solicitudId) return;
